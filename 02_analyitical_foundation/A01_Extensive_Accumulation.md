...
# A01: Extensive Margin and Structural Envelope ($K^{NRC}$)
...
### 3.1 Normal Reproduction Closure Benchmark
This corridor defines the **smooth reproduction benchmark** against which all transformation deviations are measured:
- **Condition:** $\mu_t = 1$ (full capacity utilization)
- **Implication:** Accumulation and productive capacity grow proportionally along the envelope.
- **Role:** Serves as the structural reference path. The transformation elasticity $\theta^N$ is defined relative to this benchmark, ensuring a consistent separation between structural dynamics and observed historical deviations.

**Analytical Meaning:** The rule strips away cyclical noise to reveal the institutional profitability and financial constraints that gate long-run capacity planning. It does not describe historical averages; it defines the closure-conditioned baseline for idealized coordination.
...
Although $K^{NRC}$ does not interact with $\omega$, it strictly conditions the tactical choice of technique. The mechanization productive frontier is parameterized by regime-indexed constants:
$$ a = \lambda_0^{[s]} + \lambda_1^{[s]} q + \lambda_2^{[s]} q^2 $$
Where $s$ denotes the structural regime defined by the current $K^{NRC}$ configuration and institutional closure conditions.

**Analytical Mechanism:** $K^{NRC}$ does not appear in the cost-minimization FOC for $q^*$. Instead, it shifts the frontier itself via discrete parameter jumps, altering the marginal productivity of machinery and the optimal technique $q^*(\pi, s)$ (where $\pi = 1-\omega$ is the nominal profit share, per `A02`). This cleanly separates the extensive margin (structural planning) from the intensive margin (distributive-induced innovation).
...
These deviations manifest as **low-frequency regime shifts** in the structural envelope, gating discrete breaks in the mechanization frontier parameters $\lambda_i^{[s]}$. They are not marginal adjustments but structural reorganizations of the extensive margin.

---

## Locked Formulation
$K^{NRC}$ defines the extensive margin of accumulation as a structural envelope governed by long-run reproduction conditions, financial/institutional closure, and expected trend demand. It evolves independently of short-run distributive conflict due to lumpiness, irreversibility, and multi-year planning horizons. Its analytical role is not to interact with tactical optimization, but to strictly condition the parameters of the mechanization productive frontier via regime-indexed constants ($\lambda_i^{[s]}$). The normal accumulation corridor provides the smooth reproduction benchmark ($\mu_t = 1$) against which transformation elasticity is evaluated, ensuring consistent separation between structural dynamics and historical deviations.